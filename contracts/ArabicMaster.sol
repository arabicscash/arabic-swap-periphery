...
import '@ArabicProject/arabic-swap-lib/contracts/math/SafeMath.sol';
import '@ArabicProject/arabic-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@ArabicProject/arabic-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@ArabicProject/arabic-swap-lib/contracts/access/Ownable.sol';

import './ArabicToken.sol';

contract ArabicMaster is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ABICs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accAbicPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAbicPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. ABICs to distribute per block.
        uint256 lastRewardBlock; // Last block number that ABICs distribution occurs.
        uint256 accAbicPerShare; // Accumulated ABICs per share, times 1e12. See below.
        bool exists; //
    }
    // ABIC tokens created first block.
    uint256 public abicStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when ABIC mining starts.
    uint256 public startBlock;
    // Block number when bonus ABIC period ends.
    ...
    // ABIC tokens created at bonus end block.
    uint256 public abicBonusEndBlock;
    // max reward block
    ...
    // Accumulated ABICs per share, times 1e12.
    uint256 public accAbicPerShareMultiple = 1E12;
    // The ABIC TOKEN!
    ArabicToken public abic;
    // Dev address.
    ...
    constructor(
        ...
        abic = _abic;
        devAddr = _devAddr;
        abicStartBlock = _abicStartBlock;
        ...
        // abic created when bonus end first block.
        // (abicStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        abicBonusEndBlock = abicStartBlock
        ...
        // max mint block number, _abicInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_abicInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(abicBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function add(
        ...
        pool.accAbicPerShare = 0;
        pool.exists = true;
        poolAddresses.push(_pair);
    }

    // Update the given pool's ABIC allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_abicInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            totalReward = tempTo
                ...
                .mul(_abicInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _abicInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempAbicInitBlock = _abicInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempAbicInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    // View function to see pending ABICs on frontend.
    function pendingAbic(address _pair, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfoMap[_pair];
        if (!pool.exists) {
            return 0;
        }
        UserInfo storage userInfo = poolUserInfoMap[_pair][_user];
        uint256 accAbicPerShare = pool.accAbicPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 abicReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accAbicPerShare = accAbicPerShare.add(abicReward.mul(accAbicPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accAbicPerShare).div(accAbicPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 abicReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        abic.mintTo(devAddr, abicReward.div(100));
        abic.mintTo(address(this), abicReward);
        pool.accAbicPerShare = pool.accAbicPerShare.add(abicReward.mul(accAbicPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to ArabicMaster for ABIC allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accAbicPerShare).div(accAbicPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
                safeAbicTransfer(msg.sender, pending);
            }
        }
        IBEP20(_pair).safeTransferFrom(address(msg.sender), address(this), _amount);
        userInfo.amount = userInfo.amount.add(_amount);
        userInfo.rewardDebt = userInfo.amount.mul(pool.accAbicPerShare).div(accAbicPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from ArabicMaster.
    ...
    // Safe abic transfer function, just in case if rounding error causes pool to not have enough ABICs.
    function safeAbicTransfer(address _to, uint256 _amount) internal {
        uint256 abicBal = abic.balanceOf(address(this));
        if (_amount > abicBal) {
            abic.transfer(_to, abicBal);
        } else {
            abic.transfer(_to, _amount);
        }
    }
